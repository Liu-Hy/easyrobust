...
def main(local_rank, args, args_text):
    ...
    if args.experiment:
        ...
        exp_name = '-'.join([args.resume.split('/')[-1].split('.')[0], 'vanilla', # with dat checkpoint, but no dat training
...
def train_one_epoch(
        epoch, model, loader, optimizer, loss_fn, args, best_acc, start_step, adv, delta_x, train_ratio,
...
def validate(dataloader, model, criterion, val_ratio, mask=None, adv='none', eps=2/225):
    ...
    if adv == "FGSM":
        attack = torchattacks.FGSM(model, eps=1/350)
    elif adv != "none":
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', default='../../../../pretrained/vit_base_patch16_224.pt', type=str, metavar='PATH',
